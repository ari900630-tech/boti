...
def get_main_keyboard(user_id, searching=False):
    ...
    # is_persistent=True + input_field_placeholder מבטיחים שהכפתור (4 נקודות) לא ייעלם
    return ReplyKeyboardMarkup(
        kb, 
        resize_keyboard=True, 
        is_persistent=True,
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "❌ ביטול פעולה":
        db[uid]["state"] = None
        save_db(db)
        return await update.message.reply_text("חזרנו לתפריט.", reply_markup=get_main_keyboard(user_id))
...
if __name__ == '__main__': main()       user_count = len(db.keys())
